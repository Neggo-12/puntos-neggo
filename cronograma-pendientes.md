...
**Pendiente, bloqueante para poder construir la página de Puntos Neggo:** cómo se autentica un cliente ahí. Jhey eligió "Documento + PIN temporal" (documento + un PIN de un solo uso, ej. por SMS/WhatsApp/email) — pero hoy nada llena `clientes_puntos.email`, y construir esto requiere un patrón nuevo para el proyecto: un Edge Function público, invocable directo desde el navegador de un cliente anónimo — lo que rompe a propósito la regla que se ha seguido hasta ahora ("nunca se llama desde el navegador de un cliente", ver Fase 3/7 sección de integración). Falta una conversación de diseño explícita con Jhey antes de construir/desplegar esto.

Pendiente (no bloqueante, sub-fases siguientes): solicitud de pago anticipado, y el adaptador de pago (`PaymentProvider`) con disparo manual y programado — ambos ya con decisión tomada por Jhey, falta construcción.
