...
- **Sub-tarea 3, notificación de vencimiento próximo — hecho, 17 ago (falta confirmación de Jhey con curl real).** `puntos_por_vencer_cliente` + Edge Function `puntos-por-vencer`. Ventana configurable (default 15 días, el mismo número de ejemplo de la spec -- la ventana final exacta sigue siendo decisión de Jhey). De paso se corrigió un gap real: `revertir_puntos_otorgados` (sub-tarea 2) nunca tuvo Edge Function propia -- ya desplegada (`revertir-puntos`).
