...
Deno.serve(async (req: Request) => {
  ...
  if (
    ...
    typeof valorCompra !== "number" || !(valorCompra > 0)
  ) {
    return json({
      error: "campos_invalidos",
      detalle: "tipoDocumento, numeroDocumento, comercioId, origenProducto (string) y valorCompra (number > 0) son obligatorios",
    }, 400);
  }

  const supabase = createClient(SUPABASE_URL, SERVICE_ROLE_KEY);

  const { data, error } = await supabase.rpc("otorgar_puntos", {
  ...
  if (error) {
    console.error("otorgar_puntos error", error);
